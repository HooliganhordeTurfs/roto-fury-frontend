...
import { FuryTooltip } from 'components/Pages/Bonding/FuryTooltip'
...
const Withdraw = ({
  ...
  const furyPrice = useMemo(() => {
    // @ts-ignore
    if (prices && prices?.FURY) {
      // @ts-ignore
      return prices?.FURY
    }
  ...
  const unbondingTokens = unbondingRequests?.map((row) => {
    ...
    return {
      amount: row.amount,
      tokenSymbol,
      dollarValue: (prices?.[tokenSymbol] ?? furyPrice) * row.amount,
  ...
  const withdrawableTokens = withdrawableInfos?.map((row) => ({
    ...row,
    dollarValue:
      (prices?.[row.tokenSymbol] ?? furyPrice) * row.amount,
  }))
