...
export const Dashboard: FC = () => {
  ...
  const circulatingFurySupply: number = useFetchCirculatingSupply()
  ...
  useEffect(() => {
    const marketCap = circulatingFurySupply * (prices?.FURY || 0)
    if (marketCap !== dashboardState.marketCap) {
      setDashboardDataState({ ...dashboardState,
        furyPrice: prices?.FURY ? prices.FURY : dashboardState.furyPrice,
        marketCap: marketCap ? marketCap : dashboardState.marketCap,
      })
    }
  }, [prices?.FURY, circulatingFurySupply])
