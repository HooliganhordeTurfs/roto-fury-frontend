...
export const useGetBondingAprs = () => {
  ...
  const debouncedRefetch = useMemo(() => debounce((refetchFunc) => refetchFunc(), 500),
    [])
  const furyaConfig: Config = useConfig(network, ChainId.furya)
  ...
  const { cosmWasmClient: furyaClient } = useClients(WalletChainName.furya)
  ...
  const queries = useQueries([
    {
      queryKey: ['furyaApr', network, furyaConfig],
      queryFn: () => getParamsAndCalculateApr(furyaConfig, furyaClient),
      enabled: Boolean(furyaClient) && Boolean(furyaConfig),
  ...
  const data = useMemo(() => {
    ...
    return [
      { chainName: WalletChainName.furya,
        apr: furyaApr },
